# tests/test_perf.py
# [TEST-PERF] E2.8 performance budget. Fails if find() takes more than 50 ms per 100 tokens
...
# budget, can be tightened via env var in CI
BUDGET_MS = float(os.environ.get("TARJA_PERF_BUDGET_MS", "50"))

# realistic ~90-token paragraph with a mix of identifiers and noise
SAMPLE = (
...
class TestPerf(unittest.TestCase):
    def test_budget(self):
        # [TEST-PERF] warm up, then best of 5 runs of 20 calls
    ...
    def test_sample_finds_everything(self):
        # [TEST-PERF] the sample really exercises all entities
        self.assertEqual({m.entity for m in tarja.find(SAMPLE)}, set(tarja.ENTITIES))

    @staticmethod
    def _run(n):
        # average ms per call over n calls
